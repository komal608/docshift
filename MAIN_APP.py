...
@login_required
def e_sign_page():
    return render_template('e_sign.html')
...
def convert_excel_to_pdf():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': f'Invalid Excel file: {str(excel_error)}'}), 400
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row in sheet.rows:
                ...
                if any(cell.strip() for cell in row_text if cell):  # Check if row has any non-empty content
                    has_data = True
                    text.append(' | '.join(row_text))
            
            if not has_data:
                text.append("(No data in this sheet)")
        ...
        with open(output_path, 'wb') as f:
            f.write(pdf_io.getvalue())
        return send_file(output_path, as_attachment=True, download_name=file.filename.replace('.xlsx', '.pdf').replace('.xls', '.pdf'), mimetype='application/pdf')
    except Exception as e:
...
def remove_background():
    ...
    try:
        ...
        # Return JSON with file path for frontend to use
...
@app.route('/e-sign', methods=['POST'])
@login_required
def e_sign_pdf():
    """Add digital signature/image to PDF document"""
    pdf_file = request.files.get('pdf')
    ...
    if signature_type not in ['image', 'text']:
    ...
    try:
        ...
        if signature_type == 'image':
            ...
            sig_image.save(sig_path)
        
        elif signature_type == 'text':
            signature_text = request.form.get('signature_text')
            if not signature_text:
                os.remove(filepath)
                return jsonify({'error': 'Invalid signature text'}), 400
            
            # Create image from text
            from PIL import ImageDraw, ImageFont
            sig_image = Image.new('RGBA', (400, 150), (255, 255, 255, 0))
            ...
            draw.text((20, 40), signature_text, fill=(0, 0, 0, 255), font=font)
        ...
        # Add signature to all pages (or last page if preferred)
        sig_image_for_pdf = Image.open(sig_path)
        sig_width, sig_height = sig_image_for_pdf.size
        ...
        for page_num in range(len(reader.pages)):
            ...
            # Using reportlab to overlay signature on PDF
            ...
            # Create an overlay PDF with signature
            sig_buffer = BytesIO()
            c = rl_canvas.Canvas(sig_buffer, pagesize=(page_width, page_height))
            
            # Draw signature image at bottom right
            sig_display_width = 120
            sig_display_height = int(sig_height * sig_display_width / sig_width)
            
            c.drawImage(sig_path, 
                       page_width - sig_display_width - 30, 
                       30, 
                       width=sig_display_width, 
                       height=sig_display_height)
            c.save()
            
            # Merge with current page (note: PyPDF2 may not support complex overlays)
            # For production, consider using reportlab's PdfCanvas more extensively
        ...
        # Log conversion
        log_conversion('e-sign-pdf', filename, output_filename, output_path)
        ...
        return send_file(output_path, mimetype='application/pdf', as_attachment=True, download_name='signed_document.pdf')
        
    except Exception as e:
        logger.error(f"Error during e-sign: {str(e)}")
...
def get_dashboard_data():
    """
    Fetch comprehensive dashboard data including:
    ...
    try:
        ...
        # Get total companies count (from companies table if exists, otherwise 0)
        try:
            c.execute('SELECT COUNT(*) FROM companies')
            total_companies = c.fetchone()[0] or 0
        except sqlite3.OperationalError:
            total_companies = 0
        
        # Get active companies (status = 'active')
        try:
            c.execute('SELECT COUNT(*) FROM companies WHERE status = ?', ('active',))
            active_companies = c.fetchone()[0] or 0
        except sqlite3.OperationalError:
            active_companies = 0
        ...
        # Get total files processed (from conversions table)
        c.execute('SELECT COUNT(*) FROM conversions')
        total_files = c.fetchone()[0] or 0
        
        # Get recent companies (all companies, not limited to 10)
        # FIX #1: Ensure status field is included and set to 'active' as fallback
        companies = []
        try:
            c.execute('''
                SELECT company_name, owner_name, email, phone, username, status, registered_date
                FROM companies
            ...
            companies = [
                {
                    ...
                    'phone': row[3],
                    'username': row[4],
                    'status': row[5] if row[5] else 'active',  # ✅ Ensure status is always present and defaults to 'active'
        ...
        # Get users data (for dashboard card clicks)
        # FIX #2: Ensure users_list is fully populated with id and username
        users = []
        try:
            c.execute('SELECT id, username FROM users ORDER BY id DESC LIMIT 100')
            ...
            logger.info(f"Users fetched from database: {len(users)}")
        except sqlite3.OperationalError:
            users = []
        
        # Get files/conversions data (for dashboard card clicks)
        files = []
        try:
            c.execute('''
                SELECT id, conversion_type, original_filename, converted_filename, timestamp
                FROM conversions
        ...
        # ✅ FIX #3: Ensure counts match the actual data returned
        return jsonify({
            'totalCompanies': total_companies,
            'activeCompanies': active_companies,
            'totalUsers': len(users),  # ✅ Match users array length
            'totalFiles': len(files),  # ✅ Match files array length (not COUNT)
            'companies': companies,
            'users': users,
            'files': files
        })
...
def get_companies_data():
    """
    Fetch all companies with filtering options
    Query parameters:
    ...
    try:
        ...
        try:
            if status_filter:
                c.execute('SELECT COUNT(*) FROM companies WHERE status = ?', (status_filter,))
            else:
                c.execute('SELECT COUNT(*) FROM companies')
...
def export_reports():
    ...
    Expected JSON body:
    {
        "type": "companies|users|plans|usage|financial|comprehensive",
        "fromDate": "YYYY-MM-DD",
        "toDate": "YYYY-MM-DD",
        "format": "csv|pdf",
        "planFilters": ["free", "standard", "premium"]
    }
    """
    try:
        data = request.get_json()
        report_type = data.get('type', 'companies')
        ...
        if report_type == 'companies':
            c.execute('SELECT company_name, owner_name, email, phone, username, status, registered_date FROM companies')
            columns = ['Company Name', 'Owner', 'Email', 'Phone', 'Username', 'Status', 'Date']
        elif report_type == 'users':
            c.execute('SELECT username FROM users')
            columns = ['Username']
        else:
            c.execute('SELECT * FROM conversions LIMIT 100')
            columns = ['ID', 'Type', 'Original', 'Converted', 'Timestamp']
        ...
        if export_format == 'csv':
            ...
            response = app.response_class(
                ...
                status=200,
                mimetype='text/csv',
                headers={"Content-Disposition": f"attachment;filename=docshift_report_{report_type}.csv"}
            )
            return response
            ...
            title = Paragraph(f"DocShift {report_type.title()} Report", styles['Title'])
            story.append(title)
            ...
            return send_file(
                ...
                mimetype='application/pdf',
                as_attachment=True,
                download_name=f'docshift_report_{report_type}.pdf'
            )
