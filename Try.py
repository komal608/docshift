...
FIREBASE_CRED_PATH = 'docshift.json'  # Update with your actual path
FIREBASE_DB_URL = 'https://docshift-86065-default-rtdb.firebaseio.com/'  # Update with your database URL
if not firebase_admin._apps:
    cred = credentials.Certificate(FIREBASE_CRED_PATH)
    firebase_admin.initialize_app(cred, {'databaseURL': FIREBASE_DB_URL})

cloudinary.config(
    cloud_name = 'dvdeflyta',  # Update with your Cloudinary cloud name
    api_key = '568435982421747',        # Update with your Cloudinary API key
    api_secret = '-xqsm00d0D9Hxp1YsrA6OrU-hpw'   # Update with your Cloudinary API secret
)


# Store file/image URL in Firebase Realtime Database under admin folder structure
def store_url_in_firebase(url, category, filename):
    import re
    from firebase_admin import db
    # Replace illegal characters in filename for key
    safe_key = re.sub(r'[./#$\[\]]', '_', filename)
    ref = db.reference(f'admin/{category}/{safe_key}')
    # Store both original filename and url as value
    ref.set({
        'filename': filename,
        'url': url
    })
...
def convert_image_to_pdf():
    ...
    try:
        ...
        # Upload to Cloudinary (admin/files_downloaded)
        cloudinary_url = upload_to_cloudinary(output_path, 'admin/files_downloaded')
        # Store Cloudinary URL in Firebase Realtime Database (admin/files_downloaded)
        store_url_in_firebase(cloudinary_url, 'files_downloaded', output_filename)
    except Exception as e:
...
def download():
    ...
    # Determine category by file extension
    ext = os.path.splitext(file_name)[1].lower()
    if ext in ['.pdf', '.docx', '.xlsx', '.pptx', '.txt', '.csv']:
        category = 'files_downloaded'
    elif ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.webp']:
        category = 'img_downloaded'
    elif ext in ['.mp3', '.wav', '.aac', '.ogg', '.flac']:
        category = 'audio_downloaded'
    else:
        category = 'other_downloaded'

    # Upload to Cloudinary (admin/category)
    cloudinary_url = upload_to_cloudinary(file_path, f'admin/{category}')

    # Store Cloudinary URL in Firebase Realtime Database (admin/category)
...
def convert_word_to_pdf():
    ...
    allowed = filename.endswith('.docx') or filename.endswith('.txt')
    if not allowed:
        return jsonify({'error': 'Only .docx and .txt files are supported'}), 400

    # Always use a unique temp file for input to avoid lock issues
    import tempfile
    ...
    try:
        if filename.endswith('.docx'):
            # Try using docx2pdf for best compatibility
            try:
                import pythoncom
                ...
                if not os.path.exists(output_path):
                    logger.error('docx2pdf did not produce output file.')
                    return jsonify({'error': 'docx2pdf failed to convert. Make sure Microsoft Word is installed and docx2pdf is available.'}), 500
            except Exception as e:
                logger.error(f'docx2pdf failed: {str(e)}. Falling back to text-only conversion.')
                try:
                    doc = Document(input_path)
                    text = [para.text for para in doc.paragraphs if para.text.strip()]
                    if not text:
                        logger.error('No text extracted from DOCX. The file may contain only images or unsupported content.')
                        return jsonify({'error': 'No text extracted from DOCX. The file may contain only images or unsupported content.'}), 400
                    pdf_io = BytesIO()
                    ...
                    logger.error(f'Fallback text conversion failed: {str(e2)}')
                    return jsonify({'error': f'Word to PDF conversion failed: {str(e2)}'}), 500
        elif filename.endswith('.txt'):
            try:
                with open(input_path, 'r', encoding='utf-8') as txt_file:
                    text = txt_file.read()
                if not text.strip():
                    logger.error('No text found in TXT file.')
                    return jsonify({'error': 'No text found in TXT file.'}), 400
                pdf_io = BytesIO()
                c = canvas.Canvas(pdf_io, pagesize=letter)
                y = 750
        ...
        try:
            ...
            logger.warning(f"Error deleting temp input file {input_path}: {str(e)}")
...
def remove_background():
    ...
    try:
        ...
        if not os.access(OUTPUT_FOLDER, os.W_OK):
            logger.error(f"No write permission for {OUTPUT_FOLDER}")
            return jsonify({'error': f'No write permission for output directory: {OUTPUT_FOLDER}'}), 500
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as temp_file:
            ...
            logger.debug(f"Saving output to temp file: {temp_file_path}")
            output_image.save(temp_file_path, format='PNG')

        # Copy to OUTPUT_FOLDER for logging consistency
        output_path = os.path.join(OUTPUT_FOLDER, output_filename)
        logger.debug(f"Copying temp file to: {output_path}")
        with open(temp_file_path, 'rb') as temp_f, open(output_path, 'wb') as f:
            f.write(temp_f.read())

        # Upload to Cloudinary (admin/img_downloaded)
        cloudinary_url = upload_to_cloudinary(temp_file_path, 'admin/img_downloaded')

        # Store Cloudinary URL in Firebase Realtime Database (admin/img_downloaded)
        store_url_in_firebase(cloudinary_url, 'img_downloaded', output_filename)
...
def split_pdf():
    ...
    # Upload both split PDFs to Cloudinary (admin/files_downloaded)
    cloudinary_url1 = upload_to_cloudinary(path1, 'admin/files_downloaded')
    cloudinary_url2 = upload_to_cloudinary(path2, 'admin/files_downloaded')
    # Store Cloudinary URLs in Firebase Realtime Database (admin/files_downloaded)
    store_url_in_firebase(cloudinary_url1, 'files_downloaded', output_filename1)
    store_url_in_firebase(cloudinary_url2, 'files_downloaded', output_filename2)
...
def generate_tts():
    ...
    try:
        ...
        # Upload to Cloudinary and store in Firebase immediately
        ext = os.path.splitext(filename)[1].lower()
        supported_audio_exts = ['.mp3', '.wav', '.aac', '.ogg', '.flac']
        if ext in supported_audio_exts:
            try:
                cloudinary_url = upload_to_cloudinary(filepath, 'admin/audio_downloaded')
                store_url_in_firebase(cloudinary_url, 'audio_downloaded', filename)
                logger.info(f"Audio uploaded to Cloudinary and Firebase: {cloudinary_url}")
            except Exception as e:
                logger.error(f"Cloudinary upload failed for generated audio: {str(e)}")
        else:
            logger.warning(f"Audio file extension not supported for Cloudinary upload: {filename}")
...
def save_transcript():
    try:
        ...
        # Upload to Cloudinary (admin/text_downloaded)
        cloudinary_url = upload_to_cloudinary(transcript_path, 'admin/text_downloaded')
        # Store Cloudinary URL in Firebase Realtime Database (admin/text_downloaded)
        store_url_in_firebase(cloudinary_url, 'text_downloaded', transcript_filename)
...
def summarize():
    try:
        ...
        # Upload to Cloudinary (admin/text_downloaded)
        try:
            cloudinary_url = upload_to_cloudinary(summary_path, 'admin/text_downloaded')
            # Store Cloudinary URL in Firebase Realtime Database (admin/text_downloaded)
            store_url_in_firebase(cloudinary_url, 'text_downloaded', summary_filename)
